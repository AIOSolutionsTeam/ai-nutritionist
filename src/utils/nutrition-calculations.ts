...
/**
 * Calculate carb requirements based on bodyweight and training intensity
 * Moderate Training: 3-5 g/kg (using 4 g/kg)
 * High Intensity: 6-10 g/kg (using 8 g/kg)
 * Elite/Extreme Endurance: 10-12+ g/kg (using 11 g/kg)
 */
export function calculateCarbGrams(
     weight: number, // in kg
     goals: string[]
): number {
     // Determine carb requirement based on activity level
     ...
     // For elite/extreme endurance athletes, could use 11 g/kg
     // This would be determined by very specific goals or activity level
     
     return weight * carbsPerKg;
}

/**
 * Calculate macronutrients based on TDEE, weight, and goals
 * Protein: Based on bodyweight (1.6 g/kg baseline, 2.0 g/kg for active)
 * Carbs: Based on bodyweight (4 g/kg moderate, 8 g/kg high intensity, 11 g/kg elite)
 * Fats: 20-35% of total calories (using 25% as default, or 30% for weight loss)
 */
export function calculateMacronutrients(
     ...
     // Calculate carbs based on bodyweight
     const carbGrams = calculateCarbGrams(weight, goals);
     const carbCalories = carbGrams * 4; // 4 calories per gram of carbs
     
     // Calculate fats as percentage of total calories (20-35%, using 25% default, 30% for weight loss)
     const fatPercentage = goals.includes('weight_loss') ? 0.30 : 0.25;
...
export function calculateNutritionPlan(
     ...
     const activityLevel = determineActivityLevel(goals);
     ...
     const macronutrients = calculateMacronutrients(dailyCalories, weight, goals);
