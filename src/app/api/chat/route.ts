...
import { aiUsageService, calculateCost } from '../../../lib/ai-usage'
...
export async function POST(request: NextRequest) {
     try {
          ...
          try {
               ...
               // Track AI usage/costs (use actual tokens from response when available)
               try {
                    const usage = nutritionResponse.usage
                    if (usage) {
                         ...
                         const estimatedOutputTokens = Math.ceil((nutritionResponse.reply?.length || 0) / 4)
                         const estimatedInputTokens = 2000
                         await aiUsageService.trackUsage({
                              provider: selectedProvider as 'openai' | 'gemini',
                              modelName: selectedProvider === 'openai' ? 'gpt-4o-mini' : 'gemini-2.0-flash',
                    ...
                    console.error('[API] AI usage tracking error (non-fatal):', usageError)
               }
          } catch (providerError) {
          ...
          const informationalQuestionPatterns = [
               ...
               // Questions asking "why" about previously mentioned products (explanation requests)
               /\b(why\s+(?:these|this|those|the)\s+(?:products?|compléments?|suppléments?|complements?|supplements?))\b/i,
               /\b(pourquoi\s+(?:ces?|ces)\s+(?:produits?|compléments?|suppléments?|complements?|supplements?))\b/i,
               /\b(why\s+(?:these|this|those|the))\b/i, // Standalone "why these/this/those"
               /\b(pourquoi\s+(?:ces?|ces))\b/i, // Standalone "pourquoi ces/ce"
          ]
          ...
          // Track each product recommendation for analytics dashboard
          if (finalRecommendedProducts.length > 0) {
               for (const product of finalRecommendedProducts) {
                    try {
                         await analytics.trackEvent('product_recommended', {
                              category: 'ecommerce',
                         ...
                         console.error('[API] Product recommendation tracking error (non-fatal):', analyticsError)
                    }
               }
          }
